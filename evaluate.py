...
# Import trapezoidal integration avoiding Deprecation warnings
try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rigorous OOD-centric evaluation protocol for Out-of-Distribution and Label Noise Detection."
    )
...
def stratified_split_indices(indices: np.ndarray, labels: np.ndarray, val_fraction: float, seed: int) -> Tuple[np.ndarray, np.ndarray]:
...
def load_mlp_head(artifacts_dir: Path, device: torch.device) -> torch.nn.Module | None:
    mlp_path = artifacts_dir / "mlp_descriptor_head.pth"
    if not mlp_path.exists():
        logging.warning("Missing AdvancedMLP checkpoint: %s", mlp_path)
        return None
...
def load_cvae_model(artifacts_dir: Path, sequence_length: int, device: torch.device) -> torch.nn.Module | None:
    cvae_path = artifacts_dir / "trajectory_cvae.pth"
    if not cvae_path.exists():
        logging.warning("Missing CVAE checkpoint: %s", cvae_path)
        return None

    # FIXED: Configured strictly with output_activation="none" for 1D margin regression
    cvae = TrajectoryCVAE(
...
# =======================================================
# SCORING FUNCTIONS (Must return strict 1D arrays)
# =======================================================
...
def score_trajectory(features: np.ndarray, generator: TrajectoryGenerator, batch_size: int, sequence_length: int, device: torch.device) -> np.ndarray:
    certainty_batches = []
    for start in range(0, features.shape[0], batch_size):
...
def score_mlp(features: np.ndarray, mlp: torch.nn.Module, batch_size: int, device: torch.device) -> np.ndarray:
    certainty_batches = []
    for start in range(0, features.shape[0], batch_size):
        batch = torch.from_numpy(features[start : start + batch_size]).to(device)
        with torch.no_grad():
            out = mlp(batch)
            # FIXED: Avoid multidimensional flattening of 4D targets. Extract AUM (index 0) as certainty.
            if out.shape[-1] == 4:
                certainty_batches.append(out[:, 0].cpu().numpy().astype(np.float32))
            elif out.shape[-1] == 2:
...
def score_cvae(features: np.ndarray, cvae: torch.nn.Module, batch_size: int, num_samples: int, sequence_length: int, device: torch.device) -> np.ndarray:
    certainty_batches = []
    for start in range(0, features.shape[0], batch_size):
        batch = torch.from_numpy(features[start : start + batch_size]).to(device)
        ...
        with torch.no_grad():
            sample = cvae.generate(batch_repeated) if hasattr(cvae, "generate") else cvae(batch_repeated)[0]
            sample = sample.view(curr_batch_size, num_samples, sequence_length)
...
def build_groups(spec: DatasetSpec, data_root: str, repo_root: str | None, svhn_root: str) -> Tuple[Dataset, Dataset, Dataset, Dataset, Dataset, np.ndarray]:
    ...
    clean_fit_indices, clean_val_indices = stratified_split_indices(np.flatnonzero(~noise_mask), clean_labels, val_fraction=0.2, seed=42)
    base_name = "cifar10" if spec.name == "cifar10n" else "cifar100"

    return (
        Subset(trainset_clean, clean_fit_indices.tolist()),
        Subset(trainset_clean, np.flatnonzero(noise_mask).tolist()),
        Subset(trainset_clean, clean_val_indices.tolist()),
        build_standard_dataset("cifar100" if base_name == "cifar10" else "cifar10", root=data_root, train=False),
        build_svhn_dataset(svhn_root),
...
def run_evaluation(args: argparse.Namespace) -> None:
    ...
    mlp_head = load_mlp_head(artifacts_dir, device)
    cvae_model = load_cvae_model(artifacts_dir, args.trajectory_sequence_length, device)

    clean_fit, noisy_id, clean_id, near_ood, far_ood, train_labels = build_groups(spec, args.data_root, args.repo_root, args.svhn_root)
    ...
    class_means, precision = fit_mahalanobis(collect_outputs(clean_fit, backbone, args.score_batch_size, device).features, train_labels, spec.num_classes, args.mahalanobis_reg)

    logging.info("Computing certainty scores for all methods...")
    
    method_scores = {
        "Maximum Softmax Prob (MSP)": {"clean_id": score_msp(clean_outputs.logits), "noisy_id": score_msp(noisy_outputs.logits), "near_ood": score_msp(near_outputs.logits), "far_ood": score_msp(far_outputs.logits)},
        f"Energy Score (LogitNorm)": {"clean_id": score_logitnorm(clean_outputs.logits, args.logitnorm_tau, args.logitnorm_score), "noisy_id": score_logitnorm(noisy_outputs.logits, args.logitnorm_tau, args.logitnorm_score), "near_ood": score_logitnorm(near_outputs.logits, args.logitnorm_tau, args.logitnorm_score), "far_ood": score_logitnorm(far_outputs.logits, args.logitnorm_tau, args.logitnorm_score)},
        "Mahalanobis Distance": {"clean_id": score_mahalanobis(clean_outputs.features, class_means, precision, args.score_batch_size), "noisy_id": score_mahalanobis(noisy_outputs.features, class_means, precision, args.score_batch_size), "near_ood": score_mahalanobis(near_outputs.features, class_means, precision, args.score_batch_size), "far_ood": score_mahalanobis(far_outputs.features, class_means, precision, args.score_batch_size)},
        "Deterministic Trajectory Gen": {"clean_id": score_trajectory(clean_outputs.features, trajectory_generator, args.score_batch_size, args.trajectory_sequence_length, device), "noisy_id": score_trajectory(noisy_outputs.features, trajectory_generator, args.score_batch_size, args.trajectory_sequence_length, device), "near_ood": score_trajectory(near_outputs.features, trajectory_generator, args.score_batch_size, args.trajectory_sequence_length, device), "far_ood": score_trajectory(far_outputs.features, trajectory_generator, args.score_batch_size, args.trajectory_sequence_length, device)}
    }

    if mlp_head is not None:
        method_scores["Parametric MLP Head"] = {"clean_id": score_mlp(clean_outputs.features, mlp_head, args.score_batch_size, device), "noisy_id": score_mlp(noisy_outputs.features, mlp_head, args.score_batch_size, device), "near_ood": score_mlp(near_outputs.features, mlp_head, args.score_batch_size, device), "far_ood": score_mlp(far_outputs.features, mlp_head, args.score_batch_size, device)}

    if cvae_model is not None:
        method_scores["Stochastic Trajectory CVAE"] = {"clean_id": score_cvae(clean_outputs.features, cvae_model, args.score_batch_size, args.cvae_samples, args.trajectory_sequence_length, device), "noisy_id": score_cvae(noisy_outputs.features, cvae_model, args.score_batch_size, args.cvae_samples, args.trajectory_sequence_length, device), "near_ood": score_cvae(near_outputs.features, cvae_model, args.score_batch_size, args.cvae_samples, args.trajectory_sequence_length, device), "far_ood": score_cvae(far_outputs.features, cvae_model, args.score_batch_size, args.cvae_samples, args.trajectory_sequence_length, device)}

    task_pairs = [
        ("clean_id", "noisy_id", "Task 1: Clean ID vs Noisy ID"),
        ("clean_id", "near_ood", f"Task 2: Clean ID vs Near-OOD ({spec.opposite_name})"),
        ("clean_id", "far_ood", "Task 3: Clean ID vs Far-OOD (SVHN)"),
    ]

    ordered_results: List[MetricResult] = []
    
    print("\n" + "="*90)
    print(" PAPER-READY BENCHMARK SUMMARY ".center(90, "="))
    print("="*90)
    
    for clean_group, target_group, task_name in task_pairs:
        print(f"\n▶ {task_name.upper()}")
        print("-" * 90)
        print(f"{'Method Name':<30} | {'AUROC ↑':<10} | {'AUPRC ↑':<10} | {'AURC ↓':<10}")
        print("-" * 90)
        ...
        for method_name, scores_by_group in method_scores.items():
            ...
            print(f"{res.method:<30} | {res.auroc:<10.4f} | {res.auprc:<10.4f} | {res.aurc:<10.4f}")
        print("-" * 90)
